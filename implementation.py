...
queries = [
    ...
    "SELECT agency, agency_name, COUNT(Agency) as requests_closed FROM requests WEHERE closed_date LIKE '12-25-%' AND agency IS NOT 'NYPD' GROUP BY agency ORDER BY COUNT(agency) DESC LIMIT 1",
...
indexes = [
    ...
    "CREATE INDEX idx_park_borough ON REQUESTS(park_borough);", # Optimization for Query 5
...
optimized_queries = [
    ...
    "SELECT complaint_type, count() as count FROM REQUESTS WHERE complaint_type LIKE '%Heat%' AND park_borough LIKE '%BROOKLYN%'", #Query 5
    ...
    "SELECT complaint_type, count() as count FROM REQUESTS GROUP BY complaint_type ORDER BY count DESC LIMIT 5;", #Query 7
